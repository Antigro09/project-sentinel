...
```bash
uv run pytest tests/ -q                    # 305 passing, 1 skipped (~14 min)
...
uv run python experiments/x64g_closure.py            # closure, 4/5 FAIL (~33 min)
```
...
**305 tests passing, 1 skipped. Level 5 built: the vocabulary anchor is
...
| X64G | `x64g_closure.py` | closure replication on fresh seeds -- FAILS, and why |
...
## X64G: the closure replication fails, and the bottleneck is the testbed

X64F was not closure. Two objections stood and both were right: the
"authored" control was a feature tally rather than a parser, and the
generator was enlarged after seed 101 was observed and then seed 101 was
reused. X64G rebuilds the control properly and runs on **seeds 401–404,
never generated before**.

### The fair authored control is exact

Hand-written contextual rules — word order, attachment, phrase patterns,
several senses per noun, development vocabulary only, no test seed and no
target form — score **1.00 overall and 1.00 on collisions.** X64F reported
this control at 0.02–0.18, and that number came from counting the induced
parser's own features off the development set.

| seed | contextual den/coll | bag-of-words | **authored** | static word-slot |
|---|---|---|---|---|
| 401 | 0.71 / 0.67 | 0.68 / 0.17 | **1.00** | 0.01 |
| 402 | 0.74 / 0.93 | 0.69 / 0.00 | **1.00** | 0.01 |
| 403 | 0.74 / 0.40 | 0.66 / 0.00 | **1.00** | 0.00 |
| 404 | 0.55 / 0.21 | 0.71 / 0.11 | **1.00** | 0.00 |

Pooled, paired by task meaning:

| comparison | n | difference | 95% CI | |
|---|---|---|---|---|
| contextual vs BOW, **collisions** | 62 | **+0.468** | (+0.339, +0.597) | excludes 0 |
| contextual vs BOW, all | 536 | +0.004 | (−0.043, +0.050) | includes 0 |
| contextual vs **authored**, all | 536 | **−0.326** | (−0.366, −0.287) | excludes 0 |
| contextual vs **authored**, collisions | 62 | **−0.452** | (−0.581, −0.323) | excludes 0 |
| contextual vs static word-slot | 536 | +0.663 | (+0.621, +0.703) | excludes 0 |

**G1 passes** — the targeted contextual advantage replicates on fresh seeds
and is *stronger* than X64F's (+0.468 vs +0.359), on 4/4 seeds. **G3 passes**
— non-inferior on the unstratified population. **G9 passes** — the static
word-to-slot defect is detected.

**G2 fails.** The induced parser loses to the authored one by −0.326
overall and −0.452 on the very construction it was built for.

### Why, and what it means

The surface language is generated by a template grammar I wrote, so an
inverse of that grammar can also be written — and a perfect inverse cannot
be beaten. **Every controlled language built this way has that property.**

**The bottleneck is the testbed, not the mechanism.** A self-generated
grammar cannot demonstrate that induction beats authoring, because whoever
writes the generator can write the parser. Showing that needs variation no
static rule set can invert: noise, open vocabulary, optional and
inconsistent constructions, or real text.

**X64 is not closed. X65 is not started.**

Five of the ten closure gates were run (G1, G2, G3, G9, G10). G4–G8 —
